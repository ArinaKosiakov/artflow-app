...
import { Calendar, Trash2, Check, Pencil } from "lucide-react";
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          ...
          <p
            className={`${darkMode ? "text-gray-400" : "text-gray-600"} text-sm mb-3`}
          >
            {project.description}
          </p>
          <div
      ...
      <div className="space-y-2">
        {project.steps.map((step) => (
          <div
            key={step.id}
            className={`flex items-center gap-3 p-2 rounded-lg ${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-50"} transition-colors`}
          >
            <button
            ...
            <span
              className={`text-sm ${step.done ? `line-through ${darkMode ? "text-gray-500" : "text-gray-400"}` : `${darkMode ? "text-gray-300" : "text-gray-700"}`}`}
            >
              {step.text}
            </span>
          </div>
