...
interface AddIdeaModalProps {
  ...
  initialText?: string;
  /** i18n key for title (e.g. "gallery.editIdea.title") */
  titleKey?: string;
}
...
export default function AddIdeaModal({
  ...
  darkMode,
  initialText = "",
  titleKey = "gallery.addIdea.title",
}: AddIdeaModalProps) {
  const { t } = useTranslation();
  const [text, setText] = useState(initialText);

  // Sync text when opening in edit mode with initialText
  React.useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div
        className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col`}
      >
        {/* Header */}
        <div
        ...
        {/* Footer */}
        <div
          className={`${darkMode ? "bg-gray-700" : "bg-gray-50"} px-6 py-4 flex justify-end gap-3 border-t ${darkMode ? "border-gray-600" : "border-gray-200"}`}
        >
          <button
            onClick={handleClose}
